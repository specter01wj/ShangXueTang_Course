...
public class ValidCode extends JComponent implements MouseListener {
	...
	protected void paintComponent(Graphics g) {
		...
		/*
		 * g2d.setColor(getRandColor(200, 250)); g2d.fillRect(0, 0, width,
		 * height); g2d.setColor(getRandColor(180, 200)); g2d.drawRect(0, 0,
		 * width - 1, height - 1);
		 */
		// 绘制出验证码背景的线
		int i = 0, len = 150;
		/*
		 * for (; i < len; i++) { int x = random.nextInt(width - 1); int y =
		 * random.nextInt(height - 1); int x1 = random.nextInt(width - 10) + 10;
		 * int y1 = random.nextInt(height - 4) + 4;
		 * g2d.setColor(getRandColor(180, 200)); g2d.drawLine(x, y, x1, y1); }
		 */
