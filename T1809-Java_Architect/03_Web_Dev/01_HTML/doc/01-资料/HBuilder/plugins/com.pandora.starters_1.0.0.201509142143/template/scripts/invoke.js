...
(function(owner) {
	...
	owner.login = function(loginInfo, callback) {
		...
		window.external.UserLogin(loginInfo.account,loginInfo.password);
	...
	owner.readHistory = function(callback) {
		//return "{\"EMails\": [\"waipptt1@gmail.com\",\"waipptt@gmail.com\"]}"
		callback(eval(window.external.GetAllUsers()));
	};
	...
	owner.register = function(regInfo, callback) {
		...
		window.external.UserRegist(regInfo.account,regInfo.password);
	};
	...
	owner.forgetPassword = function(loginInfo, callback) {
		...
		window.external.ForgetPassword(loginInfo.account);
	};

  /**
     * 离线登录
     * loginInfo 参考 login 方法（如用不到不管它就行）
     * */
    owner.offlineLogin = function(loginInfo, callback) {
        window.external.offlineLogin(loginInfo.account);
    };
	/**
